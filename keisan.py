...
class class文字式計算:
    """
    入力された文字式データが、メソッドによって加工され、最終は、計算結果データとなる。
    その間の、各データを属性としました。
    """
...
def 関数_字句取り出し(文字式):
    """
    コメント：この方法について、中西さんより、文字式をスライスして、数値の継続かを判断しながら、
    リストに入れていく方法を、アドバイスいただきました。その方が、コードが簡潔で、よかったのですが、
    他のプログラミング時間の関係で、置き換えることができませんでした。
...
def 関数_丸め誤差除去(数文字):
    ...
    if len(数文字) < 10:
        ...
        if '.' not in 数文字:
            if 'E' not in 数文字: #指数表現ではない整数の桁数オーバー Eなし.なし
                数文字_頭 = 数文字[0]
                数文字_尾 = 数文字[1:16]
                
                数文字_尾 = 数文字_尾.rstrip('0')
                
                指数長さ = len(数文字)
                数文字 = 数文字_頭+'.'+数文字_尾+'E'+'+'+str(指数長さ)
                # print('699')
            else:#指数表現で小数でないの場合の、桁数オーバー　Eあり.なし
                数文字_分割_リスト = 数文字.split('E')

                E_前_数文字 = 数文字_分割_リスト[0]
                E_後_数文字 = 数文字_分割_リスト[1]

                E_後_数文字_長さ = len(E_後_数文字)
                E_前_数文字_長さ = len(E_前_数文字)


                a = E_前_数文字_長さ - 1
                E_後_数文字 = str(int(E_後_数文字) + a)
                E_前_数文字 = E_前_数文字[0]+'.'+E_前_数文字[1:全文字長さ-2-E_後_数文字_長さ]

                数文字 = E_前_数文字+'E'+E_後_数文字
            ...
            if 'E' in 数文字:#Eあり.あり
                ...
                全文字長さ = 18
                E_前_数文字 = E_前_数文字[0:全文字長さ-1-E_後_数文字_長さ]

                E_前_数文字 = E_前_数文字.strip('0')
                E_前_数文字 = E_前_数文字.strip('.')
